    ...
    def _output_file_name(self, input_file, input_path, output):
        ...
        if output is not None:
            ...
            filename = os.path.join(
                output, os.path.splitext(input_file_name)[0] + ".xml"
            )
        else:
            input_file_name = ntpath.basename(input_file)
            filename = os.path.join(
                ntpath.dirname(input_file),
                os.path.splitext(input_file_name)[0] + ".xml",
            )
        ...
        for r in concurrent.futures.as_completed(results):
            ...
            if status != 200 or text is None:
                ...
                try:
                    ...
                    with open(filename.replace(".xml", "_"+str(status)+".txt"), 'w', encoding='utf8') as tei_file:
                        if text is not None:
                            tei_file.write(text)
                        else:
                            tei_file.write("")
                ...
                try:
                    ...
                    with open(filename,'w',encoding='utf8') as tei_file:
                        tei_file.write(text)
                    print("Processing of", input_file, "is successful.")
                except OSError:
                   print("Writing resulting TEI XML file", filename, "failed")
